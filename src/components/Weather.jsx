...
const Weather = ({ water, imagen }) => {
   ...
   let iconUrl = water?.weather[0].icon
    ...
    return (
        ...
        <section className='section-card container'>
            ...
            <p className="card-icon"><img src={`http://openweathermap.org/img/wn/${iconUrl}@4x.png`} alt="icon-temp" /></p>

            <p className="card-clouds">"{water?.weather[0].description}"</p>
            <p className="speed"><i class="fa-solid fa-wind"> </i><span>Wind Speed:</span> {water?.wind.speed}m/s</p>
            <p className="Clouds"><i class="fa-solid fa-cloud"></i><span>Clouds:</span> {water?.clouds.all}%</p>
            <p className="Pressure"><i class="fa-solid fa-temperature-three-quarters"></i><span className="pressuree">Pressure:</span> {water?.main.pressure}hPa</p>
